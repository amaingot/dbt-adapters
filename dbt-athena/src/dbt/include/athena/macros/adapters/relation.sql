{% macro athena__drop_relation(relation) -%}
  ...
  {%- if is_s3_tables_catalog(relation.database) -%}
...
{% macro athena__rename_relation(from_relation, to_relation) %}
  {%- if is_s3_tables_catalog(from_relation.database) -%}
    {% do exceptions.raise_compiler_error("Athena S3 Tables does not support `ALTER TABLE ... RENAME`. dbt-athena falls back to drop + create for `s3tablescatalog/<bucket>` catalogs.") %}
  {%- endif -%}
